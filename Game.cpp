...
// Constructor initializes counters for events, riddles, and characters
Game::Game(){
...
// Reads character data from a file and populates _characterOptions.
void Game::characterFile(const char filename[]){
    ...
    // Read each subsequent line, up to maxCharacter
    while(getline(myFile, line) && _characterCount < maxCharacter){
        ...
        // Parse each field between the separators
        ...
        // Create a Player object using the parsed stats and store in character options
...
// Reads random event data from a file into the _event array.
void Game::randomEventFile(const char filename[]){
    ...
    // Read each event line while there is capacity in the array
...
// Loads riddles from a file into the _riddles array.
void Game::riddleFile(const char filename[]){
    ...
    // Read each line as "question|answer"
...
// Lets both players choose which scientist/character they want to use.
void Game:: chooseCharacter(){

    cout << " AVAILABLE SCIENTISTS" << endl;
    // Display list of character options with their stats
    ...
    // Player 1 chooses a scientist by index
    cout << "Player 1, Choose your scientist (1-" << _characterCount << "): ";
    cin >> choice1;

    // Validation loop for Player 1
    while (choice1 < 1 || choice1 > _characterCount) {
        cout << "Invalid choice. Try again: ";
        cin >> choice1;
    }

    // Player 2 chooses a scientist (must be different from Player 1's choice)
    ...
    // Assign chosen characters to players (convert to 0-based index)
...
// Asks each player to choose their "path" type and stores it in their Player object.
void Game::choosePath() { 
    ...
    for (int i = 0; i < 2; i++){
       cout << "\nPath selection for Player " << (i + 1)
        ...
        // Store the player's chosen path type
        _players[i].setPathType(choice);
    }

    // Apply path-based stat changes
    pathBonus();

}

// Based on each player's chosen pathType, adjust initial stats and discovery points.
void Game::pathBonus(){
...
// Handles all DNA-related tile tasks based on tile color.
void Game:: dnaTask(int player_index, char color) {
    ...
    // BROWN TILE: DNA Task 4 (transcription)
    else if (color == 'T') {
        cout << "Brown tile: DNA Task 4 - Transcribe DNA to RNA\n";
        cout << "Enter DNA strand: ";
        cin >> s1;

        // Convert DNA to RNA and print the result
        transcribe(s1);
...
// Chooses the next random event from the list in a cyclic manner and applies it.
void Game::triggerRandomEvent(int player_index) {
    ...
    // Note: The code currently only prints changes but does not actually
    // modify the player's discover points here.
}

// Presents a riddle to the player and checks their answer.
void Game::triggerRiddle(int player_index){
    ...
    // static index cycles through the riddle array
    ...
    // playerAnswer and correctAnswer are unused here;
    // currently there is no input being read into playerAnswer.
    string playerAnswer = "";
    string correctAnswer = "";

    // This comparison is not using actual input; left unchanged.
...
// Decides what happens when a player lands on a tile based on its color.
void Game::tileEffect(int player_index, char color){

    // static variable remembers whether last green tile had an event or not
    static int greenEvent;

    switch(color) {
        case 'G':
            cout << "Green tile: 50$ chance of random event" << endl;
            // greenEvent toggles between 0 and 1 to simulate 50% chance
            if(greenEvent == 0){
        ...
        case 'R':
        case 'T':
                // All DNA-related colors handled by dnaTask
                dnaTask(player_index, color);
                break;
        case 'U':
                // Purple tile triggers riddle
                triggerRiddle(player_index);
                break;
        default:
            // For start tile, finish tile, or any undefined color
...
// Calculates final game score for a player using their DP and stats.
int Game::finalScore(Player p) const {

    int total = p.getDiscoverPoints();

    // Every 100 points in each stat translates to +1000 score
...
// Prints both players' scores and announces winner or tie.
...
// Runs the main turn-based loop where both players move until both finish.
void Game::play() { 

    // Initialize the board's tile layout
    _board.initializeBoard();

    // Track whether each player has reached the finish line
    bool finish[2] = {false, false};

    // Continue until both players are finished
    while(!(finish[0] &&finish[1])){
        // Each iteration, both players take a turn
        for (int i = 0; i < 2; i++) {
        ...
        // Move this player forward by 1 tile
        bool endReached = _board.movePlayer(i);

        // Display the board after movement
        _board.displayBoard();

        // Get player's new position and tile color
        int position = _board.getPlayerPosition(i);

        char color = _board.getTileColor(i, position);

        // If they reached the end, mark as finished
        if(endReached){
...
// High-level entry: loads all data files, setup, then runs the game.
void Game::run() { 

    // Load characters, events, and riddles from text files
    characterFile("characters.txt");
    randomEventFile("random_events.txt");
    riddleFile("riddles.txt");

    // Let players configure their characters and paths
    chooseCharacter();
    choosePath();
